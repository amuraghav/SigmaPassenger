...
//static NSString *baseUrlForXXHDPIImage = @"http://108.166.190.172:81/doctor_app/pics/xxhdpi/";

@interface CKCalendarView () <CKCalendarHeaderViewDataSource, CKCalendarHeaderViewDelegate, UITableViewDataSource, UITableViewDelegate,RadioButtonDelegate>
...
- (void)setDate:(NSDate *)date animated:(BOOL)animated
{
    ...
    if ([[self delegate] respondsToSelector:@selector(tableviewIsReloded)]) {
        [[self delegate] tableviewIsReloded];
    }
    //  Update the index
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(_isComingFrom == YES)
    {
        //static NSString *cellIdentifier=@"Cell";
        CKCalendarEvent *event = [[self events] objectAtIndex:[indexPath row]];
        CKAppointmentCell *cell= [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
        
        if(cell==nil)
        {
            cell =[[CKAppointmentCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
            cell.selectionStyle = UITableViewCellSeparatorStyleSingleLineEtched;
        ...
        NSString *strImageUrl = [NSString stringWithFormat:@"%@%@",baseUrlForXXHDPIImage,[event image]];
        
        [cell.docApptImage sd_setImageWithURL:[NSURL URLWithString:strImageUrl]
                      placeholderImage:[UIImage imageNamed:@"signup_profile_image.png"]
                             completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType,NSURL *imageurl) {
                             }];
        [cell.activityIndicator stopAnimating];
        
        [cell.docApptName setText:[event name].uppercaseString];
         [cell.docApptName setTextColor:[UIColor blackColor]];
     //   NSString *amount =  [PatientGetLocalCurrency getCurrencyLocal:[flStrForObj([event amount]) floatValue]];
        cell.totalAmount.text = [NSString stringWithFormat:@"$ %@",[event amount]];
          cell.totalAmount.textColor=[UIColor blackColor];
        [cell.appDateTime setText:[event time]];
         [cell.appDateTime setTextColor:BLACK_COLOR];
        [cell.docApptAddr setText:[event pickAdd]];
         [cell.docApptDropAddr setTextColor:[UIColor blackColor]];
        cell.distance.text = [NSString stringWithFormat:@"%@ Miles",[event distance]];
         cell.distance.textColor=BLACK_COLOR;
        [cell.docApptDropAddr setText:[event desAdd]];
        
        if ([event desAdd].length ==0) {
            
            cell.imgDropLocation.hidden=YES;
        }
        else{
           cell.imgDropLocation.hidden=NO;
        }
        
          [cell.docApptAddr setTextColor:[UIColor blackColor]];
        [cell.status setText:[event status].uppercaseString];
         cell.status.textColor=BLACK_COLOR;
        cell.imgPickLocation.textColor=[UIColor blackColor];
        cell.imgDropLocation.textColor=[UIColor blackColor];
        return cell;
    }
    else
    {
    
    CKPatientCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    
    if (cell == nil)
    {
        cell = [[CKPatientCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
        cell.backgroundColor = [UIColor clearColor];
    ...
    {
        [cell.radioButton setGroupID:100 AndID:indexPath.row AndTitle:@"title"];
        cell.radioButton.delegate =self;
    }
    else {
        [cell.radioButton setGroupID:100+indexPath.row AndID:200+indexPath.row AndTitle:@"title"];
        [cell.radioButton.btn_RadioButton sendActionsForControlEvents:UIControlEventTouchUpInside];
        cell.radioButton.delegate = Nil;
    }
...
-(void)stateChangedForGroupID:(NSUInteger)indexGroup WithSelectedButton:(NSUInteger)indexID{
     ...
     CKCalendarEvent *event = [[self events] objectAtIndex:indexID];
    [[NSUserDefaults standardUserDefaults] setObject:[event title] forKey:@"time"];
}

...
{    
    ...
    [btn_RadioButton setImage:[UIImage imageNamed:@"payment_checkmark.png"] forState:UIControlStateSelected];
...
- (void)setGroupID:(NSUInteger)indexGroup AndID:(NSUInteger)indexID AndTitle:(NSString *)title
{
    ...
    if (indexGroup == 0) {
        TELogInfo(@"this id : %lu", (unsigned long)indexID);
    }
    
    TELogInfo(@"radiobtn----------- : %lu %lu %ld", (unsigned long)indexGroup, (unsigned long)indexID, (long)self.tag);
}

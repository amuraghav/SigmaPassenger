...
{
    NSUInteger n = nameArr.count;
    NSUInteger s = subtitleArr .count;
    NSUInteger img = imageArr.count;
    
    NSUInteger totalCount = MAX(MAX(n, s), img);
...
- (void)uiSetUp
{
   ...
   _tableMenu.separatorColor = [UIColor blackColor];
    UITapGestureRecognizer *tap=[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tableTapped:)];
    [_tableMenu addGestureRecognizer:tap];
...
{
    CGPoint location = [sender locationInView:_tableMenu];
    NSIndexPath *indexPath = [_tableMenu indexPathForRowAtPoint:location];
        //Call Delegate
    if([_delegate respondsToSelector:@selector(saDropMenu:didClickedAtIndex:)]) {
        [_delegate saDropMenu:self didClickedAtIndex:indexPath.row];
    }
